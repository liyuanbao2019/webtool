...
import com.li.jc.webtool.config.SshSecurityConfig;
import com.li.jc.webtool.dto.ScriptScanResult;
...
public class SshScriptSecurityHandler {
    ...
    public Decision check(WebSocketSession session, WebSshData webSshData,
        ...
        // 执行脚本内容扫描
        com.li.jc.webtool.dto.ScriptScanResult scanResult = scriptSecurityService.scanCommand(commandText);
        ...
        if (securityConfig.isBlockCriticalScriptOps() && "high".equals(riskLevel)) {
            String blockMsg = String.format(
                    ...
                    "\r\n如确需执行，请联系管理员授权。\r\n",
                    com.li.jc.webtool.service.ScriptSecurityService.getRiskLevelLabel(riskLevel),
     ...
     * 格式化危险操作列表为终端显示格式
     */
    private String formatDangerousOpsForDisplay(List<com.li.jc.webtool.dto.ScriptScanResult.DangerousOperation> operations) {
        if (operations == null || operations.isEmpty()) {
            return "未知危险操作";
        }
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < Math.min(operations.size(), 5); i++) {
            com.li.jc.webtool.dto.ScriptScanResult.DangerousOperation op = operations.get(i);
    ...
    private void sendScriptConfirmRequest(WebSocketSession session,
                                          com.li.jc.webtool.dto.ScriptScanResult scanResult,
        ...
        try {
            ...
            confirmRequest.put("riskLevelLabel", com.li.jc.webtool.service.ScriptSecurityService.getRiskLevelLabel(scanResult.getRiskLevel()));
            confirmRequest.put("riskLevelColor", com.li.jc.webtool.service.ScriptSecurityService.getRiskLevelColor(scanResult.getRiskLevel()));
